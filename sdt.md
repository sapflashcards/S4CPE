...
graph LR
    ...
    D --> D1[Shell copy of production system without data]
    D --> D2[Convert shell to S/4HANA]
    D --> D3[Best for mostly reused processes]
    
    E --> E1[New S/4HANA installation]
    E --> E2[Transfer elements of existing configuration and ABAP repository]
    E --> E3[Best if majority of solution is being redesigned]
    
    A --> F[Partner Considerations]
    F --> F1[Partners use their own tools]
    F --> F2[SAP does not certify 3rd party tools]
    F --> F3[Issues from non-SAP tools not covered by SAP Support]
    
    style A fill:#FF5733,stroke:#333,stroke-width:2px
    style B fill:#0F9D58,stroke:#333
